...
const LoaderProvider: React.FC<LoaderProviderProps> = ({ children }) => {
  const [fullScreenLoader, setFullScreenLoader] = useState<boolean>(true)
  const [navbarLoader, setNavbarLoader] = useState<boolean>(true)

  const showFullScreenLoader = () => setFullScreenLoader(true)
  const hideFullScreenLoader = () => setFullScreenLoader(false)
